...
void toList() {
    ...
    for (int k = 0; k < chemMineLen; k++) {
        ...
        if (chemMineTemp == 43) {
            ...
            complexResult.push_back(ChemItem(0, 0));
        }
            ...
            complexResult.push_back(ChemItem(-1, 0));
...
void toMatrixBaby() {
    ...
    for (int index = 0; index < complexResult.size(); ++index) {
        ...
        if (val == ChemItem(0, 0) || val == ChemItem(-1, 0)) {
...
int main() {
    char ga[] = "H2+Ca(CN)2+NaAlF4+FeSO4+MgSiO3+KI+H3PO4+PbCrO4+BrCl+CF2Cl2+SO2=PbBr2+CrCl3+MgCO3+KAl(OH)4+Fe(SCN)3+PI3+Na2SiO3+CaF2+H2O";
    setChem(ga);
    toMatrixBaby();


 /*   for (int i = 0; i < myRow; i++) {
        for (int j = 0; j < myColume; j++) {
            cout << baby[i][j]<<endl;
        }
    }*/
